function genProto {
    ...
    protoc --proto_path="$PROTO_PATH" --go_out="$GO_OUT_PATH" --go_opt=paths=source_relative \
    ...
    protoc --proto_path="$PROTO_PATH" --grpc-gateway_out=paths=source_relative,grpc_api_configuration="$PROTO_PATH"/"${DOMAIN}".yaml:"$GO_OUT_PATH" "${DOMAIN}".proto
